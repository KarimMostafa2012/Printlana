...
<tr>
    ...
    <td class="action-section" data-title="<?php esc_attr_e('Actions', 'dokan'); ?>">
        ...
        $assigned_vendors = get_post_meta($product->get_id(), '_assigned_vendor_ids', true);
        $assigned_vendors = is_array($assigned_vendors) ? array_map('intval', $assigned_vendors) : [];
        $is_assigned_to_you = in_array($current_vendor_id, $assigned_vendors, true);

        if ($current_vendor_id === $vendor->get_id()): ?>
        ...
        <?php elseif ($is_assigned_to_you): ?>
            <button class="dokan-btn" disabled data-product="<?php echo esc_attr($product->get_id()); ?>">
                <?php esc_html_e('Assigned to you', 'printlana'); ?>
            </button>

        <?php elseif ($duplicator->check_already_cloned($product->get_id(), $current_vendor_id)): ?>
            <button class="dokan-btn" disabled data-product="<?php echo esc_attr($product->get_id()); ?>">
