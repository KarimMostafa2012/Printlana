jQuery(document).ready(function($) {
  ...
  $('#job-log-clear-button').on('click', function() {
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.success) {
          $message.text('Logs cleared successfully').css('color', '#46b450');
          setTimeout(function() {
            location.reload();
          }, 1000);
        } else {
          console.error('Failed to clear job logs:', response.data);
          $message.text('Failed to clear logs').css('color', '#dc3232');
        }
      },
      error: function() {
        console.error('AJAX error while clearing job logs');
        $message.text('Error clearing logs').css('color', '#dc3232');
      },
      complete: function() {
