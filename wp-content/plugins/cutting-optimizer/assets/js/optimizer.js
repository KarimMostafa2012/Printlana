(function($) {
    ...
    class CuttingOptimizer {
        ...
        calculateMixedOrientation() {
            ...
            const maxHorizontalStrips = Math.floor(this.sheetHeight / (this.boxHeight + this.gap));
            
            for (let hStrips = 0; hStrips <= maxHorizontalStrips; hStrips++) {
                const horizontalHeight = hStrips > 0 ? hStrips * (this.boxHeight + this.gap) - this.gap : 0;
                const remainingHeight = this.sheetHeight - horizontalHeight - (hStrips > 0 ? this.gap : 0);
                
                if (remainingHeight < 0) continue;
                
                const vStrips = Math.floor(remainingHeight / (this.boxWidth + this.gap));
                
                const boxesPerHStrip = Math.floor(this.sheetWidth / (this.boxWidth + this.gap));
                const hBoxes = hStrips * boxesPerHStrip;
                
                const boxesPerVStrip = Math.floor(this.sheetWidth / (this.boxHeight + this.gap));
        ...
        calculateAllLayouts() {
            ...
            const layout1 = this.calculateSingleOrientation(
                this.boxWidth, this.boxHeight, this.sheetWidth, this.sheetHeight
            );
            layouts.push({
            ...
            const layout2 = this.calculateSingleOrientation(
                this.boxHeight, this.boxWidth, this.sheetWidth, this.sheetHeight
            );
            layouts.push({
            ...
            const layout3 = this.calculateSingleOrientation(
                this.boxWidth, this.boxHeight, this.sheetHeight, this.sheetWidth
            );
            layouts.push({
            ...
            const layout4 = this.calculateSingleOrientation(
                this.boxHeight, this.boxWidth, this.sheetHeight, this.sheetWidth
            );
            layouts.push({
    ...
    function renderResults(optimizer) {
        ...
        layouts.forEach((layout, index) => {
            ...
            if (layout.type === 'single') {
                html += `
                    <p><strong>Grid:</strong> ${layout.cols} columns × ${layout.rows} rows</p>
                    <p><strong>Box Dimensions:</strong> ${layout.boxWidth} × ${layout.boxHeight} mm</p>
    ...
    function renderVisualDiagram(layout, optimizer) {
        ...
        if (layout.type === 'single') {
            const scale = Math.min(600 / optimizer.sheetWidth, 400 / optimizer.sheetHeight);
            ...
            html += `
                <div class="co-sheet" style="width: ${displayWidth}px; height: ${displayHeight}px;">
                    <div class="co-sheet-label">${optimizer.sheetWidth} × ${optimizer.sheetHeight} mm</div>
                    <div class="co-box-grid" style="grid-template-columns: repeat(${layout.cols}, ${boxDisplayWidth}px); gap: ${optimizer.gap * scale}px;">
            `;
            ...
            html += `<p style="color: #646970; font-style: italic;">Visual diagram available for single-orientation layouts</p>`;
        }
    ...
    $(document).ready(function() {
        $('#calculate-btn').on('click', function() {
            ...
            setTimeout(function() {
                const optimizer = new CuttingOptimizer(boxWidth, boxHeight, sheetWidth, sheetHeight, gap);
