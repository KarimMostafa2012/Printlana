		...
		if ($fetchType === 'regex') {
			...
			foreach ($matchesScriptTags as $matchedScript) {
				if (isset($matchedScript[0]) && $matchedScript[0]) {
					...
					$scriptType    = Misc::getValueFromTag($originalTag, 'type', 'dom_with_fallback') ?: 'text/javascript'; // default
                    $scriptAltType = Misc::getValueFromTag($originalTag, 'data-alt-type', 'dom_with_fallback');

                    if ($scriptType !== 'text/javascript' || ($scriptAltType && $scriptAltType !== 'text/javascript')) {
						continue;
					}
