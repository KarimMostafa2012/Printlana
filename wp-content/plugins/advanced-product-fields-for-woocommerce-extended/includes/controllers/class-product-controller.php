...
namespace SW_WAPF_PRO\Includes\Controllers {
    ...
    class Product_Controller
    {
        ...
        public function set_variant_data( $variant_data, $product, $variation ) {
            $variant_data['tax'] = wc_get_price_to_display( $variation, ['qty' => 1, 'price' => 1] );
            $variant_data['taxFactor'] = Helper::get_tax_multiplier( $variation );
            return $variant_data;
        }
 ...
 $clones[  $i - 2 ][] = $cart_item_field;
        ...
        public function display_fields_on_cart_and_checkout( $item_data, $cart_item ) {
            ...
            if( isset( $cart_item['_wapf_child'] ) && ! empty( $cart_item['_wapf_child']['parent'] ) ) {
                $title          = WC()->cart->get_cart()[ $cart_item['_wapf_child']['parent'] ]['data']->get_title();
                $item_data[]    = [
                    'key'       => __( 'Included with' , 'sw-wapf' ),
                    'value'     => $title
                ];
            }
