 ...
 * Fix: fixed an issue where the "disabled days" setting wouldn't save when "Sunday" was the only selection.
...
= version 3.0 =
 * New: new field type "Products" which allows selecting linked (child) products from your store (allowing stock management and building bundles!).
